---
title: TkxStepper
description: Stepper — accessible React component from tekivex-ui. Variants, props, and accessibility notes.
---

```tsx
import { TkxStepper } from 'tekivex-ui';
```

Stepper is part of the 78-component tekivex-ui library. Every component is
WCAG 2.1 AAA compliant, fully typed, and tree-shakeable.

## Props

| Prop | Type | Required | Description |
|---|---|---|---|
| `steps` | `Step[]` | yes |  |
| `activeStep` | `number` |  |  |
| `orientation` | `StepperOrientation` |  |  |
| `variant` | `'default' \| 'outlined' \| 'filled'` |  |  |
| `size` | `'sm' \| 'md' \| 'lg'` |  |  |
| `clickable` | `boolean` |  |  |
| `onStepClick` | `(index: number) => void` |  |  |
| `showStepNumbers` | `boolean` |  |  |
| `alternateLabel` | `boolean` |  |  |
| `connector` | `'solid' \| 'dashed' \| 'dotted'` |  |  |
| `className` | `string` |  |  |
| `style` | `CSSProperties` |  |  |

## Accessibility

- Keyboard navigation supported on all interactive elements
- `:focus-visible` outline at 2px (theme-token `primary`)
- ARIA roles and properties applied per WAI-ARIA 1.2
- Respects `prefers-reduced-motion`
- Minimum 44×44 touch target where applicable

## Source

The implementation lives at [`src/components/TkxStepper.tsx`](https://github.com/007krcs/tekivex-ui/blob/master/src/components/TkxStepper.tsx).

> This page is a generated scaffold. It will be replaced with hand-authored
> examples and a live preview in a follow-up release. For interactive demos,
> see the [legacy demo site](https://ui.tekivex.com/#/components/stepper).
